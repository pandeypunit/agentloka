...
\title{\textbf{Register Once, Verify Everywhere:}\\[0.25em] Identity for Autonomous Agents on the Open Web}
\author{Punit Pandey\\[0.15em]\normalsize AgentLoka}
\date{March 31, 2026}
...
AgentLoka is not intended to solve every identity problem. It is \textbf{not}:
...
\begin{tikzpicture}[
  ...
  actor/.style={draw, rounded corners, minimum width=2.8cm, minimum height=1.05cm, align=center, fill=black!3, draw=black!55},
...
\caption{AgentLoka protocol flow. An agent registers once with the registry, then uses short-lived proof tokens when interacting with platforms.}
...
\begin{tikzpicture}[
  ...
  box/.style={draw, rounded corners, minimum width=3.7cm, minimum height=1.15cm, align=center, fill=black!3, draw=black!55},
  secret/.style={draw, rounded corners, minimum width=3.7cm, minimum height=1.05cm, align=center, fill=black!8, draw=black!55},
  token/.style={draw, rounded corners, minimum width=3.7cm, minimum height=1.05cm, align=center, fill=black!12, draw=black!55},
  flow/.style={-{Latex[length=2.5mm]}, thick},
  risky/.style={-{Latex[length=2.5mm]}, thick, dashed, black!70},
...
\end{tikzpicture}
\caption{Credential boundary in AgentLoka. The registry secret remains confined to the agent--registry relationship, while platforms only receive short-lived proof tokens.}
...
\textbf{AgentID} positions itself as a decentralized open standard for AI agent identity, with portable metadata, trust levels, and on-chain registration. Relative to AgentID, AgentLoka is narrower in scope. It does not attempt to solve discovery, reputation, or on-chain trust. It instead concentrates on low-friction application authentication.

\textbf{Identity Registry} frames the problem around tamper-proof cryptographic directories, owner binding, and ledger-anchored accountability. Relative to that model, AgentLoka again chooses easier deployability over stronger cryptographic decentralization and legal-owner binding.
...
\bibitem{agentauth}
P. Pandey.
\newblock AgentLoka project repository.
\newblock \url{https://github.com/pandeypunit/agentloka}
...
\newblock \url{https://github.com/pandeypunit/agentloka/blob/main/docs/oauth-comparison.md}

\bibitem{platformtrust}
P. Pandey.
\newblock Platform Verification --- Do We Need It?
\newblock \url{https://github.com/pandeypunit/agentloka/blob/main/docs/platform-verification.md}
